...
async function reconcileDailyLogInventoryMovements(client, farmId) {
  await client.query(
    `WITH starter_feed AS (
       SELECT ii.farm_id, ii.id AS item_id
       FROM inventory_items ii
       WHERE ii.farm_id = $1
         AND lower(ii.name) = lower('Starter Feed')
     )
  ...
  await client.query(
    `DELETE FROM inventory_movements im
     ...
     WHERE b.farm_id = $1
       AND im.source_type = 'daily_log_feed'
  ...
  await client.query(
    `WITH doc_chicks AS (
     ...
     WHERE b.farm_id = $1
       AND im.source_type = 'daily_log_mortality'
       AND im.source_id = dl.id::text
       AND (
         dl.mortality <= 0
...
async function importDailyLogs(client, req, farmId, rows, stats, employeeIdMap = new Map(), itemIdMap = new Map()) {
  ...
  await reconcileDailyLogInventoryMovements(client, farmId);
