...
function parseQuickEntry(text, options = {}) {
  ...
  const cashAdvanceRecipient = isCashAdvance(originalText)
    ? extractCashAdvanceRecipient(originalText)
    : "";
  const receivablePaymentPayer = type === "Payment"
    ? extractReceivablePaymentPayer(originalText)
    : "";
  const description = cashAdvanceRecipient
    ? `${cashAdvanceRecipient} Cash Advance`
    : buildDescription(originalText, amountResult, categoryMatch);
  ...
  return {
    ...
    paidBy: receivablePaymentPayer || options.paidBy || "Rolly",
    paidTo: options.paidTo || cashAdvanceRecipient || "",
    reference: "",
    remarks: "",
...
function isReceivablePayment(text) {
  const hasCounterpartySignal = /\b(?:customer|buyer|client)\b/i.test(text);
  const hasPaymentSignal = /\b(?:paid|payment|bayad|nagbayad)\b/i.test(text);
  const hasReceivableSignal = /\b(?:balance|receivable|utang|kulang)\b/i.test(text);
  const hasDirectBalancePayment =
    /\bpaid\s+(?:[0-9][0-9,]*(?:\.[0-9]+)?\s+)?(?:balance|receivable)\b/i.test(text) ||
    /\b(?:balance|receivable)\s+(?:paid|payment)\b/i.test(text) ||
    /\b(?:bayad|nagbayad)\s+(?:sa\s+)?utang\b/i.test(text) ||
    /\butang\s+(?:paid|payment|bayad|nagbayad)\b/i.test(text);

  return hasPaymentSignal && hasReceivableSignal && (hasCounterpartySignal || hasDirectBalancePayment);
}

function isCashAdvance(text) {
  return /\bcash\s+advance\b|\badvance\s+cash\b|\bca\b/i.test(text);
}

function extractCashAdvanceRecipient(text) {
  const cleaned = removeDatePhrases(String(text || ""))
    .replace(/\b(?:cash\s+advance|advance\s+cash|ca)\b/gi, " ")
    .replace(/\b(?:php|pesos?|cash|gcash|bank|card|credit|debit|amount|total|for)\b/gi, " ")
    .replace(/\u20b1/g, " ")
    .replace(/\b[0-9][0-9,]*(?:\.[0-9]+)?\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function extractReceivablePaymentPayer(text) {
  const cleaned = removeDatePhrases(String(text || ""))
    .replace(/\b(?:customer|buyer|client)\b/gi, " ")
    .replace(/\b(?:paid\s+balance|paid|payment|bayad|nagbayad|balance|receivable|utang|kulang|sa)\b/gi, " ")
    .replace(/\b(?:si|ni|kay)\b/gi, " ")
    .replace(/\b(?:php|pesos?|cash|gcash|bank|card|credit|debit|amount|total|for)\b/gi, " ")
    .replace(/\u20b1/g, " ")
    .replace(/\b[0-9][0-9,]*(?:\.[0-9]+)?\b/g, " ")
    .replace(/\s+/g, " ")
